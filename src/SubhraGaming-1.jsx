...
function Loader({ done }) {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      setPct(p => {
        const next = clamp(p + Math.random() * 18, 0, 100);
        if (next >= 100) { clearInterval(id); setTimeout(done, 300); }
        return next;
      });
    }, 120);
  ...
  return (
    <div style={{ background:T.bg, minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", fontFamily:FONT_DISPLAY, color:T.text }}>
      <GlobalStyle />
      <div style={{ fontSize:14, letterSpacing:3, color:T.faint, marginBottom:14 }}>SUBHRA GAMING</div>
      <div style={{ fontSize:40, fontWeight:700, marginBottom:20 }}>{Math.floor(pct)}%</div>
      <div style={{ width:220, height:2, background:T.border }}>
        <div style={{ width:`${pct}%`, height:"100%", background:T.accent, transition:"width 0.15s" }} />
      </div>
    </div>
  );
}
...
function Hero() {
  ...
  return (
    <section style={{ minHeight:"100vh", display:"flex", flexDirection:"column", justifyContent:"center", padding:"120px 40px 60px", maxWidth:1100, margin:"0 auto" }}>
      ...
      <h1 style={{ fontFamily:FONT_DISPLAY, fontWeight:700, fontSize:"clamp(48px,9vw,120px)", lineHeight:0.95, letterSpacing:"-2px", color:T.text, margin:"0 0 24px", opacity:show?1:0, transform:show?"none":"translateY(30px)", transition:"all 0.7s 0.15s" }}>
        SUBHRA<br/>GAMING
      </h1>
...
function ProjectCard({ project, delay }) {
  ...
  return (
    <div ref={ref} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{ background:T.surface, borderRadius:8, padding:"24px 22px", border:`1px solid ${hov?T.borderStrong:T.border}`, transition:"all 0.25s", opacity:vis?1:0, transitionDelay:`${delay}ms` }}>
...
function VideoCard({ id, title, views, category, url, delay }) {
  ...
  return (
    <div ref={ref} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{ background:T.surface, borderRadius:8, overflow:"hidden", cursor:"pointer", border:`1px solid ${hov?T.borderStrong:T.border}`, transition:"all 0.25s", opacity:vis?1:0, transitionDelay:`${delay}ms` }}>
